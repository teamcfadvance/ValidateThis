...
<cfcomponent output="false" extends="BaseFileReader" hint="I am a responsible for reading and processing a JSON file.">
	...
	<cffunction name="loadRules" returnType="void" access="public" output="false" hint="I read the validations JSON file and reformat it into private properties">
		...
		<cfif isJSON(fileContent)>
			...
			<cfelse>
				<cfthrow type="ValidateThis.core.fileReaders.Filereader_JSON.invalidJSON" detail="The json object in the file #arguments.fileName# does not contain a validateThis struct." />
			</cfif>
		<cfelse>
			<cfthrow type="ValidateThis.core.fileReaders.Filereader_JSON.invalidJSON" detail="The contents of the file #arguments.fileName# are not valid JSON." />
		</cfif>
