...
<cfif StructKeyExists(Form,"Processing")>
	...
	<cfset Result = application.ValidateThis.validate(objectType="newValTypes",theObject=form,Context=Form.Context) />
...
<cfset RequiredFields = application.ValidateThis.getRequiredFields(objectType="newValTypes",Context=Form.Context) />
...
<cfif NOT Form.NoJS>
	...
	<cfset ValidationScript = application.ValidateThis.getValidationScript(objectType="newValTypes",Context=Form.Context) />
	<cfhtmlhead text="#ValidationScript#" />
</cfif>

<cfoutput>
<h1>ValidateThis New Validation Types Demo</h1>
...
<form action="index.cfm" id="frmMain" method="post" name="frmMain" class="uniForm">
	<input type="hidden" name="ObjectType" id="ObjectType" value="newValTypes" />
